...
List all patient details, diagnoses, and every single lab test, medication, and procedure mentioned.

For medications, transcribe the name, then on the same line, clearly state the dosage, frequency, and duration exactly as written (e.g., Amlopine 10 - 1x1x90).
...
// --- MODIFIED: Stage 2: Instructions for the "Open-Ended Lawyer" for GPT-4o ---
...
Patient Summary: Begin with a detailed paragraph summarizing the patient's condition, diagnoses, and key clinical findings.

Overall Assessment: Provide your expert opinion on the quality of care. Highlight correct decisions, critical omissions, and any incorrect procedures. Explain your reasoning in detail.

Detailed Analysis: Discuss each significant medication, lab, and procedure. For each item, explain its justification (or lack thereof), potential risks, and alignment with the standard of care. If there are dosing errors or unnecessary prescriptions, explain why in detail.

Recommendations: Conclude with a clear list of actionable recommendations, categorizing them by priority (e.g., Urgent, Best Practice).
...
// Note: We removed the response_format for json_object
}),
});
const data = await response.json();
if (!response.ok) throw new Error(OpenAI error: ${JSON.stringify(data)});
// We now directly return the text content
