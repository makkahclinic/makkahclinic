// pages/api/DranalyzeRx.js
// Lightweight clinical rules engine for Rx OCR lists (Next.js API Route)
...
function checkBPHWithBP(items) {
  ...
  if (hasDuodart && hasBPDrugs) {
    return {
      ...
      detail: "تامسولوسين (Duodart) قد يزيد الدوخة/هبوط الضغط مع خافضات الضغط الأخرى. راقب السقوط والضغط.",
      refs: ["JALYN_LABEL"],
    };
  }
  return null;
...
function checkPPIDuration(items, durationDays) {
  ...
  if (durationDays >= 90) {
    return {
      ...
      detail: "الاستخدام المزمن يحتاج مبررات واضحة؛ 8 أسابيع شائعة ثم إعادة التقييم.",
...
function analyzePrescription({
  ...
  patient = {}          // {age, sex, eGFR, smoker, visualSymptoms, apapDailyMg, ppiDurationDays}
}) {
...
// (اختياري) للتصدير الداخلي إن احتجته بملف آخر
