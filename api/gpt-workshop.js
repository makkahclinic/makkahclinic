...
async function getClinicalAnalysis(fullExtractedText) {
    const systemPrompt = `You are a Senior Clinical Physician. Analyze the provided patient history ("Visit Cards") and write a **deeply insightful clinical analysis**. 
...
async function getFinalReport(clinicalAnalysis, insuranceAnalysis, patientName) {
    const systemPrompt = `You are the Chief Medical Officer (CMO). You have received two detailed reports: one from your top physician and one from your best insurance auditor. Your task is to **synthesize** these into a single, cohesive, high-level professional report for the management team, formatted beautifully in Markdown.

**Mandatory Final Report Structure:**
1.  **ملخص استشاري للحالة (Executive Case Summary):**
    - **الاسم:** ${patientName}
    - [Write a powerful opening paragraph that summarizes the most critical clinical and administrative findings.]

2.  **التحليل السريري المتعمق (In-Depth Clinical Analysis):**
    - [Integrate, rephrase, and present the full clinical analysis here. Ensure it flows naturally.]

3.  **التحليل الإداري والتأميني (Administrative & Insurance Analysis):**
    - [Integrate, rephrase, and present the full insurance analysis here. Ensure it is sharp and to the point.]

4.  **التوصيات النهائية المتكاملة (Final Integrated Recommendations):**
    - [Combine the recommendations from both reports into a final, prioritized list for both the **Clinical Team** and the **Admin/Billing Team**.]`;
