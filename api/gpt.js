...
/**
 * The ultimate, final, and robust thinking process for the AI model.
 * This definitive version masters both handwriting deciphering and dosage notation analysis.
 */
...
**مهارتك الأساسية المزدوجة هي: 1) فك شفرة خط اليد الطبي الرديء. 2) فك شفرة ترميز الجرعات. نجاحك الكامل يعتمد على هاتين القدرتين.**
...
2.  **استخدم التشخيصات كمفاتيح للحل:** إذا كان التشخيص هو "ارتفاع ضغط الدم (HTN)"، فابحث بجد عن أسماء أدوية ضغط شائعة. إذا كان التشخيص "سكري"، فابحث عن أسماء مثل (Metformin, Gliclazide). استخدم معرفتك الطبية للربط بين التشخيص والدواء المحتمل.
3.  **أنشئ قائمة دقيقة بالاسم والجرعة:** لكل دواء، اذكر اسمه الذي فككت شفرته، ثم اذكر **الترميز المكتوب للجرعة** بجانبه (مثال: '1x1x90').
4.  **ترجمة ترميز الجرعة (CRUCIAL):** هذا هو مفتاح كشف الأخطاء الدقيقة. بعد كتابة الترميز، قم بترجمته إلى نص مفهوم.
    - مثال: **'1x1x90'** يعني **'حبة واحدة مرة واحدة يومياً لمدة 90 يوماً'**.
    - مثال: **'1x2x90'** يعني **'حبة واحدة مرتين يومياً لمدة 90 يوماً'**.
    - إذا لم تفهم الترميز، اذكر ذلك. هذا جزء حاسم من التحليل لكشف أخطاء الجرعات لاحقاً.
5.  **أضف درجة ثقة:** بجانب كل "اسم دواء" قمت بفك شفرته، ضع درجة ثقة في قراءتك (مثال: "Amlodipine 10mg - تم فك شفرة الاسم بثقة 95%").
6.  بعد إنشاء القائمة، اذكر بوضوح أي **نواقص جوهرية** أخرى (العمر، التاريخ، إلخ).

**الخطوة 2: التحليل الطبي المتعمق (بناءً على قائمتك التي فككت شفرتها)**
1.  الآن، انظر إلى قائمتك المكتملة (الأسماء والجرعات المترجمة).
2.  ابحث بصرامة عن **الأخطاء الطبية الجسيمة (Major Red Flags)**:
    - **الازدواجية العلاجية:** هل يوجد دوائين لعلاج نفس الشيء بطريقة خاطئة (مثال: 3 أدوية ضغط)؟ كن حاسماً.
    - **أخطاء الجرعات:** بناءً على ترجمتك للجرعة، هل هي صحيحة؟ (مثال: هل جرعة دواء `MR`/`TR` ممتد المفعول مكتوبة "مرتين يومياً"؟ هذا خطأ فادح).
    - **المكملات غير المبررة:** هل يوجد مكملات غذائية (مثل أدوية المفاصل) التي لا يغطيها التأمين؟
3.  انقد بقوة وبشكل مباشر أي خطأ تجده.
...
- بناءً على الأخطاء التي اكتشفتها والمكملات ونقص التبريرات، صنّف كل دواء حسب خطر الرفض واشرح السبب بدقة.

**الخطوة 5: صياغة توصيات تنفيذية (Actionable Recommendations)**
- **لا تكتفِ بالقول "يجب توضيح الأدوية".**
- **قدم خطة تنفيذية واضحة:**
    1.  "الفحوصات الفورية المطلوبة": اذكر قائمة الفحوصات من الخطوة 3.
    2.  "الخطة العلاجية المقترحة": اقترح خطة جديدة ومحسنة. مثال: "نوصي بإيقاف دواء [X] و [Y] فوراً بسبب الازدواجية الخطرة، والاستعاضة عنهما بدواء [Z] المركب. كما نوصي بتصحيح جرعة دواء [W] لتصبح مرة واحدة يومياً بدلاً من مرتين".

**الخطوة 6: التحليل المالي الاستراتيجي (Financial Impact)**
- لا تخترع أرقاماً. حلل التأثير المالي هيكلياً.
- "الخسائر المتوقعة": تكلفة الأدوية التي سيتم رفضها.
- "الإيرادات الإضافية الممكنة": تكلفة الخدمات المبررة التي اقترحتها.

**المخرج النهائي: (مهم جداً)**
- يجب أن يكون ردك هو كود HTML فقط، منسق وجاهز للعرض. ابدأ مباشرة بالوسم \`<h3>\`.
`;
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const payload = {
            ...
            generationConfig: {
                temperature: 0.2, // Further lowered for maximum accuracy and consistency
                topP: 0.95,
                topK: 40,
            },
        };
