// This Next.js config increases the allowed request body size
export const config = {
  api: {
    bodyParser: {
      sizeLimit: "50mb",
...
async function geminiUploadBase64({ name, mimeType, base64 }) {
    ...
    // Step 1: Start the resumable session to get an upload URL
    const initRes = await fetch(`${GEMINI_FILES_URL}?key=${encodeURIComponent(GEMINI_API_KEY)}`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "X-Goog-Upload-Protocol": "resumable",
            // ✅ [FIXED] The missing header that caused the error has been re-added
    ...
    if (!initRes.ok) {
        throw new Error(`Gemini data upload failed: ${JSON.stringify(await parseJsonSafe(initRes))}`);
    }
    ...
    // Step 2: Upload the actual file data to the session URL
...
// --- Step 1: Aggregate Clinical Data with Gemini ---
...
// --- Step 2: Get Expert Audit Instructions for GPT-4o ---
