...
async function aggregateClinicalDataWithGemini({ text, files }) {
  ...
  // **تم تحسين هذا الجزء**: تعليمات أكثر دقة لـ Gemini
...
function getExpertAuditorInstructions(lang = 'ar') {
  ...
  // **تم تحسين هذا الجزء**: تم إضافة قاعدة جديدة لتحليل الجرعات
...
function renderHtmlReport(structuredData, files, lang = 'ar') {
    ...
    // **تم تحسين هذا الجزء**: عرض الملفات المصدرية في التقرير
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // **تم تحسين هذا الجزء**: تمرير الملفات إلى دالة إنشاء التقرير
