...
// =============== ULTIMATE ENHANCEMENTS v9 (EXPERT LEVEL) ===============
// 1. Incorporated user's expert clinical feedback directly into the prompt.
// 2. Added strict rules for geriatric safety (hypoglycemia, fall risk).
// 3. Forced model to resolve ambiguous drug names (e.g., Form XR -> Metformin XR).
// 4. Added rule to differentiate drugs from procedures based on prescribing shorthand (e.g., "od x90").
// =======================================================================
...
- **قاعدة التفريق بين الدواء والإجراء:** إذا كُتب بجانب بند "od", "bid", "tid" أو "x90", "x30"، فهذا دليل قاطع على أنه **دواء** وليس إجراءً تشخيصيًا، حتى لو كان الاسم غير واضح.
- **قاعدة الاستنتاج الصيدلاني:** إذا كان اسم الدواء غير واضح (مثل Form XR)، حاول استنتاج المادة الفعالة الأكثر احتمالًا بناءً على السياق (مثل Metformin XR)، واذكر هذا الاستنتاج مع درجة الثقة (مثال: "Metformin XR - مستنتج").
- **قاعدة كبار السن (Geriatrics):** لأي مريض عمره > 65 عامًا، قم بالتحقق الإجباري من المخاطر التالية:
  1.  **خطر نقص السكر:** عند وجود أدوية Sulfonylurea (مثل Diamicron/Gliclazide)، يجب إصدار تحذير قوي وواضح من خطر نقص السكر واقتراح خطة متابعة دقيقة.
  2.  **خطر السقوط:** عند وجود دوائين أو أكثر يخفضان الضغط (مثل ACEi, ARB, CCB, مدرات البول, Tamsulosin في Duodart)، يجب إصدار تحذير قوي وواضح من خطر هبوط الضغط الانتصابي والسقوط.
- **قاعدة التشخيص الحذر:** عند استنتاج تشخيص من خط يد غير واضح (مثل Osteoarthritis/Osteoporosis)، اذكر دائمًا أن هذا تقدير وقد يكون غير دقيق.

[تحليل التداخلات الدوائية (DDI) - قاعدة إلزامية]
- في عمود "التداخلات"، لا تكتب أبدًا "مع أدوية أخرى". يجب أن تكون دقيقًا.
...
- مثال: "مع Amlodipine: زيادة خطر هبوط الضغط. يجب مراقبة الضغط بانتظام."
- إذا لم يوجد تداخل مهم، اكتب "لا يوجد تداخل سريري مهم".
