...
const EVIDENCE_DB = {
  // التهاب الجفن: النظافة والكمادات الدافئة أساس العلاج
  blepharitis_first_line: {
    summary: "إرشاد AAO/StatPearls: نظافة الجفن والكمادات الدافئة نهج أولي؛ تُستعمل المضادات الموضعية في حالات مختارة.",
    urls: [
      "https://www.aao.org/Assets/92ce1735-7c83-45f0-8a58-72997bfc2375/638442007744570000/blepharitis-ppp-2-22-24-pdf", // AAO PPP Blepharitis (2024 PDF)
      "https://www.ncbi.nlm.nih.gov/books/NBK459305/" // StatPearls Blepharitis
    ]
  },
  // مكسيفلوكساسين موضعي (VIGAMOX): الاستطباب الرسمي
  moxi_indication_conjunctivitis: {
    summary: "ملصق FDA (DailyMed): VIGAMOX® مُستطب لعلاج التهاب الملتحمة الجرثومي.",
    urls: [
      "https://dailymed.nlm.nih.gov/dailymed/drugInfo.cfm?setid=5c14c87a-660c-4fbd-9536-08486b485505"
    ]
  },
  // جفاف العين: تدرج العلاج (TFOS DEWS II + AAO PPP Dry Eye)
  dry_eye_guidance: {
    summary: "TFOS DEWS II/AAO PPP: المزلّقات وتعديل البيئة والتثقيف خطوات مبكرة؛ التصعيد حسب الشدة.",
    urls: [
      "https://www.aaojournal.org/article/S0161-6420%2824%2900012-5/pdf",    // AAO PPP Dry Eye 2024 (PDF)
      "https://www.tearfilm.org/public/TFOSDEWSII-Executive.pdf"           // TFOS DEWS II Executive Summary
    ]
  },
  // 92015 (Refraction): تغطية Medicare (كمثال سياسة)
  refraction_policy_note: {
    summary: "CPT 92015 عادةً غير مغطّى في Medicare (خدمة روتينية). قد تختلف السياسات حسب الدافع.",
    urls: [
      "https://palmettogba.com/rr/did/8eem5f7311", // Palmetto GBA: 92015 غير مغطّى
      "https://www.aao.org/eyenet/article/back-to-basics-coding-for-refractions" // AAO EyeNet coding note
    ]
  },
  // فحص قاع العين للسكري (ADA)
  ada_diabetic_eye_exam: {
    summary: "ADA 2024: فحص عيني مُوسع أوليًا وسنويًا لمرضى السكري (حسب النتائج).",
    urls: [
...
const has = (s, arr) =>
  (s || "").toLowerCase
    ? arr.some(k => (s || "").toLowerCase().includes(k))
    : false;

function uniq(arr = []) { return Array.from(new Set(arr.filter(Boolean))); }

/* ------------------------- مخططات JSON صارمة ------------------------- */
// (A) مخطط الاستخراج الحرفي (بدون تحليل)
  ...
  return {
    ...
    schema: {
      ...
      properties: {
        ...
        items: { // أدوية/فحوص/إجراءات كما وردت حرفيًا
          type: "array",
          items: {
            type: "object",
            properties: {
              ...
              dosage_written: { type: "string" },      // اكتب "—" إن لم تُذكر
              quantity_written: { type: "string" },    // كما في النموذج (1 x 150)
              notes: { type: "string" }
            },
...
// (B) مخطط التدقيق التحليلي النهائي
        ...
        table: {
          ...
          items: {
            ...
            properties: {
              ...
              citations: { type: "array", items: { type: "string" } } // روابط أدلة
            },
            required: ["name","itemType"]
          }
        },
...
/* ------------------------- مُكوّن الرؤية: بناء محتوى الرسالة ------------------------- */
...
`.trim();
  ...
  for (const f of files || []) {
    ...
    if (mime.startsWith("image/") && base64) {
      ...
      // يظهر الاسم فقط إذا لم تكن صورة (تحويل PDF->صور يجب أن يتم في الواجهة)
...
function buildAnchorsText(extracted) {
  ...
  // Anchors عينية بحسب الكلمات المفتاحية
  if (dxText.includes("blepharitis") || dxText.includes("h01") || itemsText.includes("vigamox") || itemsText.includes("moxifloxacin")) {
    parts.push(
      `Ophthalmology—Blepharitis PPP: first‑line care is lid hygiene & warm compresses; topical antibiotics reserved for select cases. Evidence: ${EVIDENCE_DB.blepharitis_first_line.urls.join(" , ")}.`
    );
    parts.push(
  ...
  if (itemsText.includes("refraction")) {
    parts.push(
      `Refraction (CPT 92015): coverage varies by payer; e.g., Medicare often excludes routine refraction. Evidence: ${EVIDENCE_DB.refraction_policy_note.urls.join(" , ")}.`
    );
  }

  // ملحوظة لمرضى السكري (لن تُستخدم إلا إذا وجد سكري ضمن التشخيصات)
  if (dxText.includes("diabetes") || dxText.includes("dm") || dxText.includes("t2dm")) {
    parts.push(
      `ADA Standard: annual dilated eye exam for diabetes (frequency individualized). Evidence: ${EVIDENCE_DB.ada_diabetic_eye_exam.urls.join(" , ")}.`
    );
  }
  return parts.join("\n");
}

/* ------------------------- مُحرّك قواعد بعد النموذج ------------------------- */
function enrichWithRules({ extracted, audit, patientInfo }) {
  ...
  // 1) إلزام إدراج "omission" لنظافة الجفن/الكمادات عند وجود Blepharitis دون ذكرها
  ...
  // 2) إلزام تمييز VIGAMOX عند عدم توثيق التهاب ملتحمة جرثومي
  ...
  for (const row of items) {
    ...
    // 4) Refraction: ملاحظة تغطية
    if (nm.includes("refraction")) {
      row.citations = uniq([...(row.citations||[]), ...citationsFor.refraction]);
      row.insuranceDecision = row.insuranceDecision || {};
      row.insuranceDecision.justification = row.insuranceDecision.justification || "تختلف التغطية حسب الدافع؛ بعض الأنظمة تعتبرها خدمة روتينية غير مغطاة.";
    }
  }

  // 5) لو ذُكر جفاف العين، عزّز التوصيات بالمزلّقات/تثقيف
  const hasDryEyeKeyword =
    lowerAll.includes("dry eye") ||
    items.some(r => (r.name||"").toLowerCase().includes("optifresh"));
  if (hasDryEyeKeyword) {
    out.recommendations = out.recommendations || [];
    out.recommendations.push({
      priority: "أفضل ممارسة",
      description: "تعليمات استخدام المزلّقات وتعديل البيئة (تقليل الشاشات/الهواء الجاف) ومراجعة التعرض للأدوية المجفِّفة.",
      relatedItems: ["OPTIFRESH OPHTHALMIC SOLUTION"],
      citations: citationsFor.dryeye
    });
  }

  // 6) سكري (إن وجد): فحص قاع العين السنوي
  if (lowerAll.includes("diabetes") || lowerAll.includes("t2dm") || lowerAll.includes("dm")) {
    out.recommendations = out.recommendations || [];
    out.recommendations.push({
      priority: "أفضل ممارسة",
      description: "فحص قاع العين الموسع سنويًا لمرضى السكري (بحسب التقييم).",
      relatedItems: [],
...
function renderHtmlReport(structured, files, lang = "ar") {
  ...
  const recs = (s.recommendations||[]).map(rec=>{
    ...
    return `<div style="display:flex;gap:12px;padding:12px;border-radius:8px;background:#f8fafc;border-${isArabic?'right':'left'}:4px solid ${urgent?'#d93025':'#16a34a'};margin-bottom:10px">
      ...
      <div>
        <div>${rec.description||""} ${citeLinks(rec.citations)}</div>
...
export default async function handler(req, res) {
  try {
    ...
    /* --- المرحلة A: استخراج حرفي بالرؤية --- */
    ...
    /* --- المرحلة B: تدقيق تحليلي مع Anchors ديناميكية --- */
