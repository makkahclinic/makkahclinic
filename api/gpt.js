// /api/gpt.js — النسخة النهائية الشمولية (Edge-safe, لا Buffer, لا responseMimeType)
// ميزات: إظهار كل البنود (لا حذف صفوف)، تعبئة الحقول الناقصة، ذكر التخصص الطبي + إطار زمني، كشف تداخلات موسّع،
// حماية 413، retries/timeout، وتصحيح تلقائي للكلاسات والنِّسَب٪، وربط الأعراض بالخدمات (CXR/LDCT).
...
1) حلّل كافة البيانات النصّيّة والصُّوَر. إن تعارض النص مع الصورة فاذكر ذلك كملاحظة حرجة وحدّد أيّهما يُعتمد ولماذا.
2) افحص بدقة وعلى نحو صريح:
   • الازدواجية العلاجية: السكري، الضغط، مضادات التخثّر/الصفائح، المضادات الحيوية.
   • أخطاء الجرعات: XR/MR تُعطى أكثر من مرّة يوميًا، جرعات عالية في CKD، وصف 90 يوم لحالات حادّة.
   ...
   • التداخلات الدوائية الحرجة (اذكرها بالاسم):
   ...
   • تعارضات المختبر الحرجة (مثل اختلاف eGFR الحديث) — اعتبر الأحدث Red Flag واذكره في التحليل السريري.

[ب] قاعدة شمولية إلزامية للجدول (مهم جداً)
- **يجب** إدراج **كل** الأدوية والإجراءات المذكورة في النص أو الظاهرة بالصورة كصفوف في الجدول **دون حذف أي صف**.
- إن نقصت أي معلومة لعمود (الجرعة/الغرض/التداخلات/التصنيف/…)، اكتب **"غير محدد"** أو **"يحتاج توضيح"**.
- لا تستبدل “عدم المعرفة” بالحذف. لا تدمج صفّين مختلفين إلا إذا كانا مكررين تمامًا.

[ج] قواعد مخاطبة التأمين (إلزامية)
...
3) <h4>التحليل السريري العميق</h4><p>
   - اربط الأعراض بالتوجيهات: مريض ≥40 سنة ومدخّن مع سعال مزمن/فقدان وزن ⇒ CXR الآن وLDCT السنوي (إن انطبقت معايير USPSTF).
...
<!-- املأ كل الصفوف دون حذف -->
</tbody></table>

[هـ] قواعد إضافية إلزامية لعمود "الجرعة الصحيحة المقترحة"
- عند الحاجة إلى إحالة، اكتب **التخصص صراحة** + **إطار زمني مفضل** داخل نفس الخلية. أمثلة مُلزمة:
  • CKD/HyperK أو ACEi/ARB + MRA ⇒ **طبيب كلى (Nephrology) خلال 48–72 ساعة**.
  • فشل قلبي/تداخلات قلبية/تعدد مضادات التخثّر ⇒ **طبيب قلب (Cardiology) خلال أسبوع**.
  • ضبط سكري/CKD مع سكري ⇒ **طبيب غدد صماء (Endocrinology) خلال أسبوع**.
  • سعال مزمن/مدخّن/فقدان وزن ⇒ **طبيب صدّرية (Pulmonology) خلال أسبوع**.
  • شكوى بصرية/سكري مزمن ⇒ **طبيب عيون (Ophthalmology) خلال 2–4 أسابيع**.
  • شكوى جلدية/ستيرويد موضعي طويل ⇒ **طبيب جلدية (Dermatology) حسب الحاجة**.
  • نقرس/Allopurinol ⇒ **طبيب روماتيزم (Rheumatology) خلال شهر**.
  • مشاكل INR/Warfarin أو نزف/خثار ⇒ **عيادة تخثّر أو أمراض دم (Hematology/Anticoagulation) خلال أسبوع**.

[و] فرص تحسين الخدمة ورفع مستوى الدخل (وفق مصلحة المريض – مدعومة بالأدلة)
...
- فعّل البنود حين تنطبق محفزاتها:
  • سكري نوع 2 ⇒ **HbA1c** كل 3 أشهر إن غير منضبط، 6–12 أشهر إن مستقر — ADA 2025.
  • Metformin/Xigduo XR أو سكري/CKD ⇒ **eGFR + UACR** — FDA + KDIGO/ADA–KDIGO.
...
/** تصحيح ذاتي: إضافة % + تطبيق الكلاسات على خلايا بدون class + تحويل http→https + منع حذف الصفوف */
function applySafetyPostProcessing(html) {
  try {
    html = String(html || '');

    // % لو مفقودة داخل خلايا <td>
...
خفّض جودة/دقّة الصور أو استخدم Files API.`,
    ...
    const payload = {
      ...
      generationConfig: { temperature: 0.2, topP: 0.95, topK: 40 } // لا تضع responseMimeType
    };
