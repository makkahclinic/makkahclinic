...
// ============= I18N =============
const i18n = {
  ar:{hero:{title:'منصة الصيدلية الذكية',subtitle:'تجميع بيانات المريض، تحليل تفاعلات/جرعات، وتصدير تقرير PDF احترافي.',cta:'بدء تحليل جديد',viewLast:'عرض آخر تقرير'},sec:{patient:'١) بيانات المريض',organs:'٢) تقييم وظائف الأعضاء',allergy:'٣) الحساسية',symptoms:'٤) معلومات سريرية إضافية'},f:{age:'العمر',gender:'الجنس',height:'الطول (سم)',weight:'الوزن (كجم)',pregnant:'هل المريضة حامل؟',gestation:'مدة الحمل (أسابيع)',egfr:'معدل الترشيح الكبيبي (eGFR)',crea:'مستوى الكرياتينين (mg/dL)',liver:'حالة الكبد',foodAllergy:'هل توجد حساسية طعام؟',symptoms:'الأعراض الإكلينيكية',meds:'قائمة الأدوية (سطر لكل دواء أو مفصولة بفواصل)',apap:'إجمالي باراسيتامول يوميًا (mg) – اختياري',uploadLab:'تحميل نتائج المختبر',uploadRad:'تحميل تقارير الأشعة',dropHere:'اضغط لرفع ملفات (صور/PDF)'},opt:{select:'اختر',male:'ذكر',female:'أنثى',yes:'نعم',no:'لا'},liver:{normal:'سليم',mild:'قصور طفيف',moderate:'قصور متوسط',severe:'قصور حاد'},btn:{addAllergy:'+ إضافة حساسية أخرى',analyze:'تحليل وتصدير',print:'طباعة'},hint:{analyze:'سيتم توليد تقرير شامل قابل للطباعة وPDF.',security:'* لا يتم تخزين ملفاتك على الخادم؛ تستخدم فقط لتوليد التقرير محليًا.'},report:{title:'ملخص تقييم المريض',patient:'بيانات المريض',allergies:'الحساسية',files:'المرفقات',analysis:'تحليل الأدوية'}},
  en:{hero:{title:'Smart Pharmacy Portal',subtitle:'Capture patient data, analyze interactions/doses, export a professional PDF.',cta:'Start New Analysis',viewLast:'View Last Report'},sec:{patient:'1) Patient Information',organs:'2) Organ Function',allergy:'3) Allergies',symptoms:'4) Additional Clinical Information'},f:{age:'Age',gender:'Gender',height:'Height (cm)',weight:'Weight (kg)',pregnant:'Is the patient pregnant?',gestation:'Gestation (weeks)',egfr:'eGFR',crea:'Creatinine (mg/dL)',liver:'Liver status',foodAllergy:'Any food allergies?',symptoms:'Clinical Symptoms',meds:'Medications list (one per line or comma-separated)',apap:'Total paracetamol per day (mg) – optional',uploadLab:'Upload Lab Results',uploadRad:'Upload Radiology Reports',dropHere:'Click to upload files (images/PDF)'},opt:{select:'Select',male:'Male',female:'Female',yes:'Yes',no:'No'},liver:{normal:'Normal',mild:'Mild impairment',moderate:'Moderate impairment',severe:'Severe impairment'},btn:{addAllergy:'+ Add Another Allergy',analyze:'Analyze & Export',print:'Print'},hint:{analyze:'A comprehensive, printable PDF will be generated.',security:'* Your files are not stored on the server; they are used locally to build the report.'},report:{title:'Patient Assessment Summary',patient:'Patient Details',allergies:'Allergies',files:'Attachments',analysis:'Rx Analysis'}}
};
let currentLang='ar';
...
function setLanguage(lang){
  ...
  document.querySelectorAll('[data-i18n]').forEach(el=>{
    ...
    el.textContent = key.split('.').reduce((o,k)=>o&&o[k], i18n[lang]);
  });
}
...
function renderReport(data,payload){
  ...
  return `
    ...
    <div>${allHtml || '<span class="hint">'+t('None reported','لا يوجد')+'</span>'}</div>
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap"><div><b>${t('Labs','مختبر')}:</b> ${labs||'-'}</div><div><b>${t('Radiology','أشعة')}:</b> ${rads||'-'}</div></div>
...
async function doAnalyze(){
  ...
  try{
    ...
    console.error(e); showNote('err',t('Request failed: ','فشل الطلب: ')+e.message);
  }
}
...
async function downloadPDF(){
  ...
  if(!el || el.style.display==='none'){ showNote('err',t('No report to export.','لا يوجد تقرير للتصدير.')); return; }
