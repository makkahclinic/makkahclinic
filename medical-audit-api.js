...
في نهاية التقرير، يجب إضافة تقييمين رقميين من 1 إلى 10:
...
حيث:
- **تقييم الالتزام بمعايير التأمين (X)**: من 1-10 بناءً على صحة الترميز، اكتمال التوثيق، قابلية المطالبة للقبول
- **تقييم جودة الخدمات الطبية (Y)**: من 1-10 بناءً على صحة التشخيص، مناسبة العلاج، اتباع البروتوكولات

أعط التقرير بتنسيق HTML جميل مع ألوان وأيقونات واضحة.
استخدم اللون الأخضر للصحيح، الأحمر للخطأ، الأصفر للتحذيرات.`;

const MULTI_CASE_PROMPT = `أنت خبير تأمين طبي متخصص في مراجعة الحالات الطبية وتقييمها من منظور التأمين ومعايير الجودة.

مهمتك: تحليل **جميع** الحالات الطبية المرفقة (سواء كانت 5 أو 50 أو 100 حالة أو أكثر) وتقييم كل حالة بدقة عالية.

**معايير التقييم لكل حالة:**
1. مطابقة الإجراءات للبروتوكولات الطبية المعتمدة (CDC, MOH, WHO)
2. صحة الترميز (ICD-10, CPT codes)
3. توثيق الأدوية والجرعات
4. اكتمال الفحوصات المطلوبة
5. منطقية العلاج للتشخيص
6. احتمالية قبول أو رفض المطالبة من التأمين

**المطلوب إخراجه:**
جدول HTML شامل لكل الحالات مع الأعمدة:
| رقم | المريض/الحالة | التشخيص | الإجراءات/الأدوية | الترميز | الحالة | المشكلة | التصحيح المطلوب |

**تصنيف الحالات (مع تلوين الصف):**
- 🟢 **مقبول** (خلفية #dcfce7): توثيق كامل، ترميز صحيح، علاج مناسب، مطالبة ستُقبل
- 🔴 **مرفوض** (خلفية #fee2e2): أخطاء جوهرية - ترميز خاطئ، إجراءات غير مبررة، توثيق ناقص جداً، المطالبة سترفض
- 🟡 **يحتاج تصحيح** (خلفية #fef9c3): مشاكل قابلة للإصلاح مع شرح **تفصيلي** لكيفية التصحيح

**للحالات الصفراء (يحتاج تصحيح) - يجب توضيح:**
- ما هو الخلل بالتحديد؟
- ما هو التصحيح المطلوب خطوة بخطوة؟
- ما هو الترميز الصحيح (إن وجد خطأ)؟
...
✅ مقبول: Y (نسبة%)
❌ مرفوض: Z (نسبة%)
⚠️ يحتاج تصحيح: W (نسبة%)
\`\`\`

**ملاحظة:** حلل جميع الحالات بدون استثناء مهما كان عددها.
...
🔴 تعليمات إلزامية - يجب اتباعها بدقة:

1. **التحليل المعمق إجباري**: لا تختصر أبداً. كل حالة تحتاج تحليل مفصل.
2. **كل قسم يجب أن يحتوي على 3-5 نقاط على الأقل** مع شرح وافٍ.
3. **اذكر أسماء الأدوية والتحاليل والتشخيصات بالضبط** كما وردت في البيانات.
4. **لا تكتب "لا توجد معلومات كافية"** - حلل ما هو متاح بعمق.
5. **استخدم الأيقونات والألوان بكثرة** لجعل التقرير مرئياً وجذاباً.
6. **التقرير النهائي يجب أن يكون طويلاً ومفصلاً** (1500+ كلمة للحالة الواحدة).
7. **أضف جدول ملخص** في النهاية يوضح كل النقاط.

قم بتحليل البيانات أعلاه وأعط تقريراً شاملاً ومفصلاً بتنسيق HTML.` });
