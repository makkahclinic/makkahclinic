...
1. **تتبع رقم المريض (PATIENT FILE NO)**: ابحث عن أي مريض له أكثر من زيارة في نفس الفترة
2. **كشف التكرار**: إذا وجدت نفس رقم المريض بنفس التشخيص أو أدوية متشابهة خلال فترة قصيرة
3. **⚠️ صياغة تأمينية - لا اتهام!** استخدم هذه الصياغة:
   ❌ لا تكتب: "تكرار مشبوه" أو "احتيال محتمل"
   ✅ اكتب: "⚠️ رُصد تكرار خدمة [اسم الخدمة] لنفس المريض في [التاريخ].
   📌 **معرض للرفض التأميني** بسبب عدم توثيق المبرر الإكلينيكي.
   📋 **المطلوب للقبول**: توثيق سبب التكرار في الملف الطبي (مثال: عدم استجابة للجرعة الأولى، تفاقم الحالة، إلخ)
   📚 **المرجع**: بروتوكولات وزارة الصحة ومعايير CBAHI للتوثيق"

# 📏 قواعد توثيق الطول والوزن والطب الوقائي (إلزامي!)
1. **تقييم توثيق العلامات الحيوية**: تحقق من وجود الطول (Height) والوزن (Weight) في البيانات
2. **إذا غاب الطول/الوزن**: اكتب:
   "⚠️ **نقص توثيق**: لم يُسجل الطول و/أو الوزن للمريض.
   📌 هذا يُعد مخالفة لمعايير CBAHI (معيار التقييم الأولي).
   ❗ يؤثر سلباً على: حساب الجرعات الدوائية، تقييم BMI، خطة الطب الوقائي"
3. **إذا وُجد الطول/الوزن - قيّم الطب الوقائي**:
   - احسب BMI = الوزن ÷ (الطول بالمتر)²
   - إذا BMI > 25 (زيادة وزن) أو > 30 (سمنة): 
     "⚠️ المريض يعاني من [زيادة وزن/سمنة] - BMI = [الرقم]
     📋 **المطلوب وقائياً**: توثيق نصائح غذائية، إحالة لأخصائي تغذية عند الحاجة
     📌 عدم التوثيق = فرصة ضائعة للطب الوقائي"
   - إذا BMI < 18.5 (نقص وزن):
     "⚠️ المريض يعاني من نقص الوزن - BMI = [الرقم]
     📋 **المطلوب**: تقييم سوء التغذية، فحوصات إضافية"
4. **ضغط الدم (Blood Pressure)**: إذا مرتفع > 140/90:
   "⚠️ ضغط دم مرتفع [القراءة] - هل تم تقديم نصائح وقائية أو إحالة؟"
...
<!-- العلامات الحيوية والطب الوقائي - قسم إلزامي! -->
<div style="background:#f0f7ff;padding:15px;border-radius:8px;margin-bottom:15px;border-right:4px solid #2196f3;">
<h3 style="color:#1565c0;margin:0 0 10px;">📏 العلامات الحيوية والطب الوقائي</h3>
<table style="width:100%;border-collapse:collapse;">
<tr style="background:#e3f2fd;">
<th style="padding:8px;text-align:right;border:1px solid #ddd;">القياس</th>
<th style="padding:8px;border:1px solid #ddd;">القيمة</th>
<th style="padding:8px;border:1px solid #ddd;">التقييم</th>
</tr>
<tr>
<td style="padding:8px;border:1px solid #ddd;font-weight:bold;">الطول (Height)</td>
<td style="padding:8px;border:1px solid #ddd;">[القيمة من البيانات أو "⚠️ غير مسجل"]</td>
<td style="padding:8px;border:1px solid #ddd;">[التقييم]</td>
</tr>
<tr>
<td style="padding:8px;border:1px solid #ddd;font-weight:bold;">الوزن (Weight)</td>
<td style="padding:8px;border:1px solid #ddd;">[القيمة من البيانات أو "⚠️ غير مسجل"]</td>
<td style="padding:8px;border:1px solid #ddd;">[التقييم]</td>
</tr>
<tr>
<td style="padding:8px;border:1px solid #ddd;font-weight:bold;">مؤشر كتلة الجسم (BMI)</td>
<td style="padding:8px;border:1px solid #ddd;">[الحساب إن توفرت البيانات]</td>
<td style="padding:8px;border:1px solid #ddd;">[طبيعي/زيادة وزن/سمنة/نقص وزن]</td>
</tr>
<tr>
<td style="padding:8px;border:1px solid #ddd;font-weight:bold;">ضغط الدم</td>
<td style="padding:8px;border:1px solid #ddd;">[القيمة من البيانات]</td>
<td style="padding:8px;border:1px solid #ddd;">[طبيعي/مرتفع/منخفض]</td>
</tr>
<tr>
<td style="padding:8px;border:1px solid #ddd;font-weight:bold;">النبض</td>
<td style="padding:8px;border:1px solid #ddd;">[القيمة من البيانات]</td>
<td style="padding:8px;border:1px solid #ddd;">[طبيعي/سريع/بطيء]</td>
</tr>
<tr>
<td style="padding:8px;border:1px solid #ddd;font-weight:bold;">الحرارة</td>
<td style="padding:8px;border:1px solid #ddd;">[القيمة من البيانات]</td>
<td style="padding:8px;border:1px solid #ddd;">[طبيعي/حمى]</td>
</tr>
</table>

<!-- تقييم الطب الوقائي -->
<div style="margin-top:10px;padding:10px;background:#fff8e1;border-radius:6px;border-right:3px solid #ff9800;">
<strong>🩺 تقييم الطب الوقائي:</strong>
<ul style="margin:5px 0 0 0;padding-right:20px;font-size:0.95rem;">
<li><strong>توثيق العلامات الحيوية:</strong> [مكتمل ✅ / ناقص ⚠️ - مع ذكر ما ينقص]</li>
<li><strong>النصائح الوقائية:</strong> [هل وُثقت نصائح غذائية/رياضية للمريض؟ خاصة إذا BMI > 25]</li>
<li><strong>الإحالات الوقائية:</strong> [هل أُحيل لأخصائي تغذية/قلب إذا لزم؟]</li>
</ul>
</div>
</div>
