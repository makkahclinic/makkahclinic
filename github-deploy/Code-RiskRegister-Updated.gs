...
/**
 * الحصول على إحصائيات المخاطر للداشبورد
 */
function getMetrics() {
  ...
  let total = 0, high = 0, med = 0, low = 0, resolved = 0, critical = 0;
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (status === 'مغلق' || status === 'closed') {
      resolved++;
    } else {
      if (level === 'حرج' || level === 'critical') {
        critical++;
        high++;
      } else if (level === 'عالي' || level === 'high') {
        high++;
      } else if (level === 'متوسط' || level === 'medium') {
        med++;
      } else if (level === 'منخفض' || level === 'low') {
        low++;
      }
    }
  }
...
function getRisks() {
  ...
  return { success: true, ok: true, data: risks, items: risks };
}
